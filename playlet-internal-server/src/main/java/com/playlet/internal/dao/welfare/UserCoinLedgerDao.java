...
public interface UserCoinLedgerDao extends BaseMapper<UserCoinLedgerEntity> {
	...
	UserCoinLedgerEntity findByBiz(@Param("uid") Integer uid, @Param("bizType") String bizType,
			@Param("bizId") String bizId);

	@Select("select * from user_coin_ledger where uid = #{uid} order by setTime desc, id desc")
	List<UserCoinLedgerEntity> findByUid(@Param("uid") Integer uid);
	...
	@Select("<script>"
			...
			+ "</script>")
	List<UserCoinLedgerEntity> findByUidAndBizType(@Param("uid") Integer uid, @Param("bizType") String bizType);
}
