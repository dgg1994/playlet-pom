...
import com.playlet.oversea.api.response.SystemMessageItemEntity;
...
public interface UserSystemMessageDao extends BaseMapper<UserSystemMessageEntity> {
	...
	@Select("<script>"
			...
			+ "    COALESCE(i_pref.title, i_def.title, i_en.title) AS title,"
			+ "    COALESCE(i_pref.content, i_def.content, i_en.content) AS content,"
			+ "    COALESCE(i_pref.cover_url, i_def.cover_url, i_en.cover_url) AS coverUrl,"
			+ "    p.drama_id AS dramaId, p.jump_type AS jumpType,"
			+ "    COALESCE(NULLIF(i_pref.jump_param,''), NULLIF(i_def.jump_param,''), NULLIF(i_en.jump_param,''), p.jump_param) AS jumpParam,"
			...
			+ "  LEFT JOIN system_message_publish_i18n i_en ON i_en.publish_id = p.id AND i_en.langue = 'en'"
			...
			+ "    AND COALESCE(i_pref.title, i_def.title, i_en.title) IS NOT NULL"
			+ "  UNION ALL"
